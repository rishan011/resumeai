...
import {
  ...
  Target, BarChart3, Shield, Palette, ChevronRight, LayoutTemplate, PenTool, Wand2, LogOut, LayoutDashboard, Youtube, Briefcase
} from "lucide-react";
...
const SLIDES = [
  ...
  },
  {
    ...
    icon: Youtube,
    color: "red",
    title: "Summarize videos for your resume",
    description: "AI extracts key achievements and skills from technical YouTube videos in seconds.",
  },
  {
    id: "jobsearch",
    tag: "AI Job Search",
    icon: Briefcase,
    color: "blue",
    title: "Find tailored roles with Firecrawl",
    description: "Smart search that finds jobs matching your skills using deep web crawling.",
  },
];
...
function YouTubeSlide() {
  return (
    <div className="flex h-full items-stretch">
      <div className="w-1/2 border-r border-white/[0.05] p-8 bg-neutral-950/40 flex flex-col gap-5">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-8 h-8 rounded-xl bg-red-500/20 flex items-center justify-center border border-red-500/30"><Youtube className="w-4 h-4 text-red-400" /></div>
          <span className="text-sm font-bold text-white tracking-wide">YouTube Summarizer</span>
        </div>
        <div className="flex-1 rounded-2xl border border-white/[0.05] bg-neutral-900/60 p-5 flex flex-col justify-center items-center text-center">
           <div className="w-12 h-12 rounded-full bg-red-500/10 flex items-center justify-center mb-4">
              <Youtube className="w-6 h-6 text-red-500" />
           </div>
           <div className="text-xs text-neutral-400 mb-4">Paste any video URL to extract <br/> professional achievements.</div>
           <div className="w-full h-10 bg-neutral-950 border border-neutral-800 rounded-lg flex items-center px-3 text-[10px] text-neutral-600">
              https://youtube.com/watch?v=k38...
           </div>
        </div>
      </div>
      <div className="flex-1 p-8 flex flex-col gap-4">
        <div className="text-xs font-bold text-white flex items-center gap-2">
          <Sparkles className="w-3.5 h-3.5 text-amber-500" /> AI Takeaways
        </div>
        <div className="space-y-3">
          {[
            "Implemented high-concurrency microservices with Go.",
            "Reduced API latency by 45% using Redis caching.",
            "Orchestrated Kubernetes clusters for 99.9% uptime."
          ].map((text, i) => (
            <motion.div 
              key={i}
              ...
              className="p-3 rounded-xl bg-white/5 border border-white/[0.05] text-[11px] text-neutral-300 flex gap-3"
            >
              <CheckCircle className="w-3.5 h-3.5 text-emerald-500 shrink-0" /> {text}
            </motion.div>
          ))}
        </div>
      </div>
...
function JobSearchSlide() {
  return (
    <div className="flex h-full items-stretch">
      <div className="w-1/2 border-r border-white/[0.05] p-8 bg-neutral-950/40 flex flex-col gap-5">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-8 h-8 rounded-xl bg-blue-500/20 flex items-center justify-center border border-blue-500/30"><Briefcase className="w-4 h-4 text-blue-400" /></div>
          <span className="text-sm font-bold text-white tracking-wide">AI Job Search</span>
        </div>
        <div className="space-y-4">
           <div>
              <div className="text-[10px] text-neutral-500 mb-1.5 uppercase font-bold">Search Query</div>
              <div className="w-full h-10 bg-neutral-900 border border-white/5 rounded-lg flex items-center px-3 text-xs text-neutral-300">
                 Staff Frontend Engineer
              </div>
           </div>
           <div>
              <div className="text-[10px] text-neutral-500 mb-1.5 uppercase font-bold">Location</div>
              <div className="w-full h-10 bg-neutral-900 border border-white/5 rounded-lg flex items-center px-3 text-xs text-neutral-300">
                 Remote / New York
              </div>
           </div>
           <Button className="w-full bg-blue-600 hover:bg-blue-500 text-white h-10 text-xs font-bold rounded-lg shadow-lg shadow-blue-600/10">
              Scrape for Jobs →
           </Button>
        </div>
      </div>
      <div className="flex-1 p-6 flex flex-col gap-3 overflow-hidden">
         <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest pl-2">Matching Opportunities</div>
         {[
           { t: "Senior React Dev", c: "Linear", s: "$160k - $210k" },
           { t: "Staff UI Engineer", c: "Stripe", s: "$190k+" },
           { t: "Frontend Lead", c: "Vercel", s: "Equity + $175k" }
         ].map((j, i) => (
           <motion.div 
             key={i}
             ...
             className="p-3 rounded-xl bg-neutral-900 border border-white/[0.05] flex flex-col gap-1"
           >
             <div className="text-xs font-bold text-white">{j.t}</div>
             <div className="flex justify-between items-center">
                <span className="text-[10px] text-neutral-500">{j.c}</span>
                <span className="text-[10px] text-emerald-400 font-bold">{j.s}</span>
             </div>
           </motion.div>
         ))}
      </div>
...
const SLIDE_COMPONENTS = [BuilderSlide, AtsSlide, CoverLetterSlide, YouTubeSlide, JobSearchSlide];
          ...
          >
            ...
            {!session && (
              <Button 
                ...
                className="h-14 px-8 border-neutral-700 text-neutral-300 hover:bg-neutral-800 hover:border-neutral-500 rounded-xl text-base transition-all bg-neutral-900/50 backdrop-blur-sm flex items-center gap-3"
              >
            ...
            <a href="#templates">
              <Button size="lg" variant="outline" className={`h-14 px-8 border-neutral-700 text-neutral-300 hover:bg-neutral-800 hover:border-neutral-500 rounded-xl text-base transition-all bg-neutral-900/50 backdrop-blur-sm ${session ? "" : "hidden sm:flex"}`}>
                See Templates
              </Button>
            </a>
          </motion.div>
